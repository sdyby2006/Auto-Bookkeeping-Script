...
LLMDatabase.prototype.initLLMData = function () {
  ...
  this.insertLLM('qwen', 'https://metahk.zenymes.com/v1/chat/completions', 'Qwen3-235B-A22B-FP8');
...
- **描述**: 账单分类，此参数代表分类的名称。
  - 如果type为支出，则返回["餐饮美食", "交通出行", "居住生活", "购物消费", "休闲娱乐", "学习教育", "医疗健康", "人情送礼", "经营成本", "其他支出" ]中任意一个。
  - 如果type为收入，则返回[ "工资薪金", "投资收益", "租金收入", "报销退款", "赠予补贴", "兼职外快", "论文辅导", "咨询服务", "闲置变现", "其他收入"]中任意一个。
...
`
  ...
  type2Catenames:{
    "支出": [
      "餐饮美食", "交通出行", "居住生活", "购物消费", "休闲娱乐", "学习教育", "医疗健康", "人情送礼", "经营成本", "其他支出" 
    ],
    "收入": [
      "工资薪金", "投资收益", "租金收入", "报销退款", "赠予补贴", "兼职外快", "论文辅导", "咨询服务", "闲置变现", "其他收入" 
    ]
  },
  type2Int: {
    "支出": 0,
...
let paidCatenameBtnData = [
  { text: "餐饮美食", color: "#FFB300" }, // 琥珀色
  { text: "交通出行", color: "#4CAF50" }, // 绿色
  { text: "居住生活", color: "#2196F3" }, // 蓝色
  { text: "购物消费", color: "#9C27B0" }, // 紫色
  { text: "休闲娱乐", color: "#FF5722" }, // 深橙色
  { text: "学习教育", color: "#00BCD4" }, // 青色
  { text: "医疗健康", color: "#F44336" }, // 红色
  { text: "人情送礼", color: "#E91E63" }, // 粉色
  { text: "经营成本", color: "#795548" }, // 棕色
  { text: "其他支出", color: "#9E9E9E" }  // 灰色
];

let incomeCatenameBtnData = [
  { text: "工资薪金", color: "#4CAF50" }, // 绿色
  { text: "投资收益", color: "#FFC107" }, // 琥珀色
  { text: "租金收入", color: "#03A9F4" }, // 浅蓝色
  { text: "报销退款", color: "#8BC34A" }, // 浅绿色
  { text: "赠予补贴", color: "#FF9800" }, // 橙色
  { text: "兼职外快", color: "#673AB7" }, // 深紫色
  { text: "论文辅导", color: "#009688" }, // 青绿色
  { text: "咨询服务", color: "#CDDC39" }, // 柠檬绿
  { text: "闲置变现", color: "#607D8B" }, // 蓝灰色
  { text: "其他收入", color: "#B0BEC5" }  // 浅灰色
];
...
const groupById = {
  ...
  "catename": [
    ...
    { type: 'text', content: '餐饮美食', color: '#FF8300', id: 'catename' },
...
// 3. AI分析
    OrderProcessor.analyzeOrder(url, model, key, parseredOcrResult)
      .onData((dataStr) => {
        dataStr = dataStr.replace(/```json/g, "").replace(/```/g, "");
        ...
        if (data && data.accountname) {
          data.accountname = data.accountname
            ...
            .replaceAll(']', ')')
            .replace(/\s+/g, '')
            .replace(/(银联|国际|IC|白金|金卡|普卡|钻石|VISA|MasterCard|UnionPay)/g, '')
            .replace('借记', '储蓄')
            .replace(/^中行/g, '中国银行')
            .replace(/^工行/g, '工商银行')
            .replace(/^建行/g, '建设银行')
          ...
          if (accountnames.includes(data.accountname)) {
            ...
            // 模糊匹配（数字匹配）
            const matchDigits = data.accountname.match(/\d{2,}/);
            if (matchDigits) {
        ...
        if (data && JSON.stringify(data) !== JSON.stringify(lastData)) {
          ...
          if (diffData["type"]) {
            ...
            let btnData = (text === "支出") ? paidCatenameBtnData :
                          (text === "收入") ? incomeCatenameBtnData : null;
      ...
      .then((data) => {
        ...
        if (data.type && data.catename) {
          ...
          if (catenameList && !catenameList.includes(data.catename)) {
        ...
        if (!data.time) {
          ...
          if (/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}$/.test(fixedTime)) {
            fixedTime += ":00";
          } else if (/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$/.test(fixedTime)) {
            fixedTime = fixedTime.replace(/:\d{2}$/, ":00");
          } else {
            try {
              const d = new Date(fixedTime);
              if (!isNaN(d.getTime())) {
                fixedTime = formatDate(d, "yyyy-MM-dd hh:mm:00");
              } else {
